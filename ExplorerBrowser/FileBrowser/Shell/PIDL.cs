    ...
    internal class PIDL : IEnumerable
    {
        ...
        public static IntPtr ILClone(IntPtr pidl)
        {
            ...
            //if (size > 0)
            {
                ...
                newPidl = Marshal.AllocCoTaskMem(size + 2);
        ...
        {
            IntPtr nextPidl = IntPtr.Zero;

            try
            {
                int size = ItemIDSize(pidl);
                int ptr = (int)pidl + size;

                //if (ptr <= int.MaxValue)
                {
                    nextPidl = new IntPtr(ptr);
                }
            }
            catch(Exception e)
            {
        ...
        public static IntPtr ILCombine(IntPtr pidl1, IntPtr pidl2)
        {
            ...
            //if (size1 > 0)
            {
                newPidl = Marshal.AllocCoTaskMem(size1 + size2 + 2);
